...
It consumes `/api/v1/aircraft` (including its embedded GeoJSON contact collection), `/api/v1/weather`, and `/api/v1/observing`. It contains no upstream-provider logic. A static host such as GitHub Pages can serve this directory after the API host and CORS policy are selected in later Stage 21 increments.
