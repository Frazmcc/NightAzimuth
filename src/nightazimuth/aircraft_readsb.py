...
class ReadsbProvider:
    ...
    def fetch_snapshot(
        ...
        del observer, radius_km  # local endpoint defines its own receiver coverage
        ...
        try:
            ...
            return AircraftSnapshot(
                ...
                coverage_description="local receiver coverage",
                state=AircraftSnapshotState.UNAVAILABLE,
                error=f"local ADS-B receiver unavailable: {exc}",
            )
        finally:
            if owns_client:
                client.close()
        ...
        observations = tuple(
            ...
            if (observation := _normalise_readsb_record(row, source_time)) is not None
        )
...
def _normalise_readsb_record(
    ...
    try:
        return AircraftObservation(
            ...
            on_ground=on_ground,
            position_observed_at=source_time - timedelta(seconds=seen_pos),
            contact_observed_at=source_time - timedelta(seconds=seen),
            source_id=ReadsbProvider.provider_id,
            source_label=ReadsbProvider.label,
            source_kind=AircraftSourceKind.LOCAL,
        )
    except ValueError:
        return None
